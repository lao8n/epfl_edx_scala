...
object Server extends ServerModuleInterface {
  ...
  val reframedFlow: Flow[ByteString, String, NotUsed] =
    Flow[ByteString]
      .via(Framing.delimiter(ByteString("\n"), maximumFrameLength=256, allowTruncation=false)) // Flow[ByteString, BytreString, NotUsed]
      .map(_.utf8String)
        ...
        event => {
          event match {
            case Event.Follow(_, from, to) =>
              followers = followers + (from -> (followers.getOrElse(from, Set.empty) + to))
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    eventAndFollowers match {
      ...
      // No clients should be notified for Unfollow (and other events)
      case _ ⇒ false
    }
